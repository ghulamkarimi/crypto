...
const EditClosePositionJournal = () => {
  ...
  const [resultValue, setResultValue] = useState(
    journal?.results ? "Profit" : !journal?.results ? "Less" : ""
  );
  const [profitValue, setProfitValue] = useState(
    journal?.profit ? journal?.profit.toString() : ""
  );
  const [lossValue, setLossValue] = useState(
    journal?.loss ? journal?.loss.toString() : ""
  );
  const [endTimeValue, setEndTimeValue] = useState(
    journal?.endTime ? journal?.endTime : ""
  );
  const [endDateValue, setEndDateValue] = useState(
    journal?.endDate ? journal?.endDate : today
  );
  const [tradeSummeryValue, setTradeSummeryValue] = useState(
    journal?.tradeSummary ? journal?.tradeSummary : ""
  );
        ...
        <div className=" relative">
          ...
          <div className=" flex gap-2 mt-4 w-full">
            <div className={`w-1/3 ${resultValue === "Loss" && " hidden"}`}>
              <Profit
                profitValue={profitValue.toString()}
...
export default EditClosePositionJournal;
