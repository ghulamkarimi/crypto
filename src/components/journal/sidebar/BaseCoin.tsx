...
interface IBaseCoinProps {
  coinImage: string;
  placeHolder: string;
  baseCoinValue:string
  setBaseCoinValue:(baseCoinValue:string)=>void
}
      ...
      <div className="wrapper-input-coin-journal w-full">
        <input
          ...
          onChange={(event: React.ChangeEvent<HTMLInputElement>) => {
            setBaseCoinValue(event.target.value);
        ...
        {coins
          .filter(
            (coin) =>
              coin?.name && coin?.name
                .toLowerCase()
                .includes(baseCoinValue?.toLowerCase()) ||
            ...
            <ul
              className={`even:bg-LIGHT_GRAY odd:bg-DARK_GRAY cursor-pointer py-2 w-full px-4 hover:bg-LIGHT_BLUE
               ${!searchValueInputCoinActive && "hidden"}  ${
                baseCoinValue?.trim() === "" && "hidden"
              }`}
            
              key={index}
            
              onClick={() => {
                setBaseCoinValue(coin?.name);
